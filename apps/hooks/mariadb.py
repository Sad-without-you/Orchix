...
def _wait_for_mariadb(container_name: str, timeout: int = 60) -> bool:
    for _ in range(timeout):
        r = subprocess.run(
            ['docker', 'exec', container_name, 'sh', '-c',
             'mariadb-admin -u root -p"$MARIADB_ROOT_PASSWORD" ping --silent 2>/dev/null'],
            capture_output=True
        )
        if r.returncode == 0:
            return True
...
def backup(container_name: str) -> bool:
    """Backup MariaDB using mariadb-dump (logical dump, safe on running container)."""
    try:
...
def restore(backup_file: Path, container_name: str) -> bool:
    """Restore MariaDB from mariadb-dump SQL file."""
    try:
        import shutil
        ...
        # Stop and clear the data volume so MariaDB re-initializes fresh
        ...
        # Start fresh container so MariaDB initializes the data dir
        ...
        if not _wait_for_mariadb(container_name, timeout=60):
            return False
