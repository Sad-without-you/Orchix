...
def _generic_volume_backup(container_name: str) -> bool:
    ...
    try:
        ...
        # Stat commands: output one UID per line (first N lines of stdout)
        stat_cmds = '; '.join(
        ...
        volume_uids = []
        if br.stdout:
            volume_uids = [l.strip() for l in br.stdout.splitlines()
...
def _generic_volume_restore(container_name: str, backup_file: Path) -> bool:
    try:
        ...
        if not volume_owners:
            ...
            if fallback_uid != '0':
                vols_to_fix = all_volumes if is_multi else [volume_name]
        ...
        def _restore_single(vol: str, subdir) -> subprocess.CompletedProcess:
        ...
        if is_multi:
            ...
            rr = _restore_single(volume_name, None)
